...
class My_Controller extends CI_Controller
{
    ...
    /**
     * @var bool|User $user An object of User model
     */
	var $user = FALSE;
	var $client = FALSE;
	...
	function __construct()
	{
		...
		parent::__construct();
		//TODO: need to cache this in the session and read from cache
		...
		if($this->user || $this->client){
			...
			if($this->user){
				$module_list = Module::find('all',array(
					'conditions' => array("id != 12")));
				$this->_module_list = [];
				$this->view_data['title'] = '';
				foreach($module_list as $module) {
					$this->_module_list[$module->link]['id'] = $module->id;
					$this->_module_list[$module->link]['name'] = $module->name;
					$this->_module_list[$module->link]['link'] = $module->link;
					$this->_module_list[$module->link]['type'] = $module->type;
					$this->_module_list[$module->link]['icon'] = $module->icon;
					$this->_module_list[$module->link]['sort'] = $module->sort;
				}
				$this->view_data['module_list'] = $this->_module_list; 

				$access 			= $this->user->access;
				$access 			= explode(",", $access);
				$this->view_data['user_access'] = $access;
				$this->view_data['client_access'] = [];
				$this->_client_access = $this->view_data['client_access'];
				$this->_user_access = $this->view_data['user_access'];
				$update 			= $this->user;
                ...
                $this->view_data['menu'] = Module::find('all', array('order' => 'sort asc', 'conditions' => array('(id in (?) AND type = ?) OR (id in (?) AND type = ?)', $access, 'main', $access, 'team')));
			...
			$mentions = Mention::find( 'all', array( 'conditions' => array( 'mention=?', $this->user->username ), 'limit' => 50, 'order' => 'id desc' ) );
			$unread_mentions_count = Mention::count( array( 'conditions' => "mention = '{$this->user->username}' AND is_read = 0" ) );

			$this->view_data['mentions'] = $mentions;
			$this->view_data['mentions_count'] = $unread_mentions_count;
				...
				$access = explode(",", $access);
				$this->view_data['client_access'] = $access;
				$this->view_data['user_access'] = [];
				$this->_client_access = $this->view_data['client_access'];
				$this->_user_access = $this->view_data['user_access'];
			...
			}, User::find( 'all', array( 'conditions' => array( 'status=?', 'active' ) ) ));

			$this->view_data['mention_names'] = $usernames;

			//Update user last active
			$update->last_active = time();
			$update->save();

			$this->view_data['token'] = $this->security->get_csrf_hash();
			$this->view_data['token_name'] = $this->security->get_csrf_token_name();
			...
			$this->view_data['title'] = "Client View";
			$this->view_data['client_access'] = [];
			$this->view_data['user_access'] = [];
			$this->_client_access = $this->view_data['client_access'];
			$this->_user_access = $this->view_data['user_access'];
			if($this->session->userdata('logged_in')!=TRUE) {
                if (
                    ...
                    strpos($_SERVER['REDIRECT_URL'],'logout') === false
                ) {
		...
		//echo '<pre>';var_dump($this->view_data['module_list'], $this->view_data['user_access']);exit;

		if ($this->user) {
			$this->setupAccountModels();
			$this->setupAccount();
		}
	}
	...
	public function setupAccountModels() {
		...
		//TODO: take out  && 0==1 to activate billing
		if (!in_array($_SESSION['accountUrlPrefix'], $this->account->getReportingExclusionList()) && 0==1){
		...
		//TODO: take out  && 0==1 to activate billing
		if (!in_array($_SESSION['accountUrlPrefix'], $this->account->getReportingExclusionList()) && 0==1) {
			$speraPaymentDue = $this->account->getBilling()->getDue();
			if ($speraPaymentDue) {
				$amountDue = $this->account->getBilling()->getAmountDue();
			} else {
				$amountDue = 0;
			}
			if ($amountDue != 0 &&
			    $this->router->class != 'auth' &&
			    $this->router->class != 'payments' &&
			    $this->router->class != 'signup' &&
			    $this->router->class != 'propay' &&
			    $this->router->class != 'api' &&
			    $this->router->class != 'cinvoices' &&
			    $this->router->class != 'cmessages' &&
			    $this->router->class != 'cestimates' &&
			    $this->router->class != 'cprojects' &&
			    $this->router->class != 'csubscriptions' &&
			    $this->router->class != 'pinvoices'
			)  {
				redirect('payments/expiredsubscription');
			}
		}
	}
}
