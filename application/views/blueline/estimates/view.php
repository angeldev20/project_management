          ...
          <div class="row">
			...
			<?php if($estimate->estimate_status != "Accepted" && $estimate->estimate_status != "Invoiced"){ ?><a href="<?=base_url()?>estimates/item/<?=$estimate->id;?>" class="btn btn-primary" data-toggle="mainmodal"><i class="icon dripicons-plus visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_add_item');?></span></a><?php } ?>
			...
			<?php if(($estimate->estimate_status == "Open" || $estimate->estimate_status == "Revised") && is_object($estimate->company)){ ?><a href="<?=base_url()?>estimates/sendestimate/<?=$estimate->id;?>" class="btn btn-primary"><i class="icon dripicons-mail visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_send_estimate_to_client');?></span></a><?php } ?>
			<?php if($estimate->estimate_status == "Accepted"){ ?><a href="<?=base_url()?>estimates/estimateToInvoice/<?=$estimate->id;?>" class="btn btn-success"><i class="icon dripicons-mail visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_convert_to_invoice');?></span></a><?php } ?>
			<?php if($estimate->estimate_status == "Invoiced"){ ?><a href="<?=base_url()?>invoices/view/<?=$estimate->id;?>" class="btn btn-success"><i class="icon dripicons-mail visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_go_to_invoice');?></span></a><?php } ?>
		...
		<ul class="details col-xs-12 col-sm-6">
			...
			<li><span><?=$this->lang->line('application_contact');?>:</span> <?php if(is_object($estimate->company->client)){ ?><?=$estimate->company->client->firstname;?> <?=$estimate->company->client->lastname;?> <?php }else{echo "-";} ?></li>
		...
		$second_tax_value = floatval($second_tax_value);
