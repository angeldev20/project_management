	...
	<div class="col-sm-12  col-md-12 main">  
     <div class="row">
		...
		<tr id="<?=$value->id;?>" >
			...
			<td><span class="label label-info"><?php if(!is_object($value->company)){echo $this->lang->line('application_no_client_assigned'); }else{ echo $value->company->name; }?></span></td>
			...
			<td>
					<span class="label <?php if($value->status == "Active"){echo ' ';} 
							if($value->end_date <= date('Y-m-d') && $value->end_date != "" && $value->status != "Inactive"){ $ended = true; echo ' label-success tt" title="'.$this->lang->line('application_subscription_has_ended'); }elseif($value->end_date == ""){ echo ' label-success tt" title="'.$this->lang->line('application_unlimited'); } ?>">
							<?php if($value->end_date != "" ){ $unix = human_to_unix($value->end_date.' 00:00'); echo '<span class="hidden">'.$unix.'</span> '; echo date($core_settings->date_format, $unix); }else{
								echo '<i class="ion-ios-infinite row_icon"></i>';
								}?>
					</span>
			</td>
			<td class="hidden-xs"><span class="label <?php if( $value->status == "Active" && $value->next_payment > date('Y-m-d')){echo 'label-success';} if($value->next_payment < date('Y-m-d') && $value->status != "Inactive" && ($value->end_date > date('Y-m-d') || $value->end_date == "") ){ echo 'label-important tt" title="'.$this->lang->line('application_new_invoice_required'); } ?>"><?php $unix = human_to_unix($value->next_payment.' 00:00'); echo '<span class="hidden">'.$unix.'</span> '; if(($value->end_date < date('Y-m-d') && $value->end_date != "") && $value->next_payment > date('Y-m-d')){ echo $this->lang->line('application_payments_closed');}else{echo date($core_settings->date_format, $unix);}?></span></td>
			<td><span class="label <?php if($value->status == "Active"){echo 'label-success';}else{echo "label-important";} ?>"><?php if(($value->end_date <= date('Y-m-d') && $value->end_date != "") && $value->status != "Inactive"){ echo $this->lang->line('application_ended'); }else{ echo $this->lang->line('application_'.$value->status); } ?></span></td>
