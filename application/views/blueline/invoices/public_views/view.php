<div class="row">
		...
		<div class="row">
		<div class="col-md-12">
		<div class="table-head"><?=$this->lang->line('application_invoice_items');?> <?php if($invoice->estimate_status != "Invoiced"){ ?><span class=" pull-right"></span><?php } ?></div>
		...
		<tr id="<?=$value->id;?>" >
            <td class="option" style="text-align:left;" width="8%">
                <i class="btn-option icon dripicons-lock"></i>
            </td>

            <td><?php if(isset($value->name) && !empty($value->name)){echo $value->name;}else{ echo (isset($invoice->invoice_has_items[$i]->item->name)) ? $invoice->invoice_has_items[$i]->item->name : ''; }?></td>
		...
		<div class=" col-md-12" align="right">
			<?php if($core_settings->paypal == "1" && $sum != "0.00" && $invoice->status != "Paid"){ 
				...
				?>
						...
						<input type="hidden" name="return" value="<?=base_url()?>pinvoices/view/<?=$invoice->id;?>">
						<input type="hidden" name="cancel_return" value="<?=base_url()?>pinvoices/view/<?=$invoice->id;?>">
	...
	<div class="btn-group dropup">
      ...
      <?php if($core_settings->bank_transfer == "1" && $sum != "0.00" && $invoice->status != "Paid" ){ ?>
	    <li><a id="pay_bank_transfer" data-toggle="mainmodal" href="<?=base_url()?>pinvoices/banktransfer/<?=$invoice->id;?>/<?=$sumRest;?>"><?=$this->lang->line('application_bank_transfer');?></a></li>
	  <?php } ?>
      ...
      <li><a id="pay_credit_card" data-toggle="mainmodal" href="<?=base_url()?>pinvoices/propay/<?=$invoice->id;?>/<?=$sumRest;?>?paymentType=card"><?=$this->lang->line('application_credit_card');?></a></li>
      <li><a id="pay_check" data-toggle="mainmodal" href="<?=base_url()?>pinvoices/propay/<?=$invoice->id;?>/<?=$sumRest;?>?paymentType=ach"><?=$this->lang->line('application_check');?></a></li>
	  ...
	  <?php if($core_settings->stripe == "1" && $core_settings->twocheckout == "0" && $core_settings->authorize_net == "0" && $sum != "0.00" && $invoice->status != "Paid" ){ ?>  
	    ...
	    <li><a id="pay_credit_card" data-toggle="mainmodal" href="<?=base_url()?>pinvoices/stripepay/<?=$invoice->id;?>/<?=$sumRest;?>"><?=$this->lang->line('application_credit_card');?></a></li>
	    	<?php if($core_settings->stripe_ideal == "1") { ?>
	    		<li><a id="pay_ideal" data-toggle="mainmodal" href="<?=base_url()?>pinvoices/stripepay/<?=$invoice->id;?>/<?=$sumRest;?>/ideal"><?=$this->lang->line('application_iDEAL');?></a></li>
	    	<?php } ?>
	  <?php } ?>
	  ...
	  <?php if($core_settings->twocheckout == "1" && $core_settings->stripe == "0" && $core_settings->authorize_net == "0" && $sum != "0.00" && $invoice->status != "Paid" ){ ?>  
	    ...
	    <li><a id="pay_credit_card" data-toggle="mainmodal" href="<?=base_url()?>pinvoices/twocheckout/<?=$invoice->id;?>/<?=$sumRest;?>"><?=$this->lang->line('application_credit_card');?></a></li>
	  <?php } ?>

	  <?php if($core_settings->stripe == "0" && $core_settings->twocheckout == "0" && $core_settings->authorize_net == "1" && $sum != "0.00" && $invoice->status != "Paid" ){ ?>  
	    <li><a id="pay_credit_card" data-toggle="mainmodal" href="<?=base_url()?>pinvoices/authorizenet/<?=$invoice->id;?>/<?=$sumRest;?>"><?=$this->lang->line('application_credit_card');?></a></li>
	  <?php } ?> -->
	  </ul>
      <?php endif;?>
	</div>
