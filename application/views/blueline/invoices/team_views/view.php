<div class="row">
            ...
            <?php if ($user->admin != 1) : ?>
                <a href="<?=base_url()?>tinvoices/update/<?=$invoice->id;?>/view" class="btn btn-primary" data-toggle="mainmodal"><i class="icon dripicons-pencil visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_edit_invoice');?></span></a>
			    <?php if($invoice->estimate_status != "Invoiced"){ ?><a href="<?=base_url()?>tinvoices/item/<?=$invoice->id;?>" class="btn btn-primary" data-toggle="mainmodal"><i class="icon dripicons-plus visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_add_item');?></span></a><?php } ?>
                <a href="<?=base_url()?>tinvoices/payment/<?=$invoice->id;?>" class="btn btn-primary" data-toggle="mainmodal"><i class="icon dripicons-card visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_add_payment');?></span></a>
			<?php endif; ?>
			<div class="btn-group">
			  ...
			  <ul class="dropdown-menu">
			    <li><a href="<?=base_url()?>tinvoices/preview/<?=$invoice->id;?>" ><?=$this->lang->line('application_download_pdf');?></a></li>
			    <li><a href="<?=base_url()?>tinvoices/preview/<?=$invoice->id;?>/show" target="_blank"><?=$this->lang->line('application_preview_pdf');?></a></li>
			  </ul>
			</div>
			<?php if($invoice->status != "Paid" && is_object($invoice->company)){ ?><a href="<?=base_url()?>tinvoices/sendinvoice/<?=$invoice->id;?>" class="btn btn-primary"><i class="icon dripicons-mail visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_send_invoice_to_client');?></span></a><?php } ?>
		...
		<ul class="details col-xs-12 col-sm-6">
			...
			<li class="<?=$invoice->status;?>"><span><?=$this->lang->line('application_visibility');?>:</span> <?php if($invoice->issue_date <= date('Y-m-d')){ echo '<a class="label label-success" >'.$this->lang->line('application_visible_to_client').'</a>';}else{echo '<a class="tt label label-important" title="'.$this->lang->line('application_issue_date_not_reached').'">'.$this->lang->line('application_not_visible_to_client').'</a>';} ?>
		...
		<ul class="details col-xs-12 col-sm-6">
			...
			<li><span><?=$this->lang->line('application_contact');?>:</span> <?php if(is_object($invoice->company->client)){ ?><?=$invoice->company->client->firstname;?> <?=$invoice->company->client->lastname;?> <?php }else{echo "-";} ?></li>
		...
		<div class="row">
		<div class="col-md-12">
		<div class="table-head"><?=$this->lang->line('application_invoice_items');?> <?php if($invoice->estimate_status != "Invoiced"){ ?><span class=" pull-right"><a href="<?=base_url()?>tinvoices/item/<?=$invoice->id;?>" class="btn btn-md btn-primary" data-toggle="mainmodal"><i class="fa icon dripicons-plus visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_add_item');?></span></a></span><?php } ?></div>
			...
			foreach ($payments as $value) {  ?>
				...
				<tr class="sec">
					<td class="option" style="text-align:left;" width="8%">
					        ...
					        <button type="button" class="btn-option delete po" data-toggle="popover" data-placement="right" data-content="<a class='btn btn-danger po-delete ajax-silent' href='<?=base_url()?>tinvoices/payment_delete/<?=$payments[$i]->id;?>/<?=$invoice->id;?>'><?=$this->lang->line('application_yes_im_sure');?></a> <button class='btn po-close'><?=$this->lang->line('application_no');?></button> <input type='hidden' name='td-id' class='id' value='<?=$value->id;?>'>" data-original-title="<b><?=$this->lang->line('application_really_delete');?></b>"><i class="icon dripicons-cross"></i></button>
					        <a href="<?=base_url()?>tinvoices/payment_update/<?=$payments[$i]->id;?>" title="<?=$this->lang->line('application_edit');?>" class="btn-option" data-toggle="mainmodal"><i class="icon dripicons-gear"></i></a>
		...
		<div class=" col-md-12" align="right">
			<?php if($core_settings->paypal == "1" && $sum != "0.00" && $invoice->status != "Paid"){ 
				...
				?>
						...
						<input type="hidden" name="image_url" value="https://spera-<?=ENVIRONMENT ?>.s3-us-west-2.amazonaws.com/<?=$_SESSION["accountUrlPrefix"]?>/<?=$core_settings->invoice_logo;?>">
						<input type="hidden" name="amount" value="<?=$sumRest;?>">
						<input type="hidden" name="no_shipping" value="1">
						<input type="hidden" name="no_note" value="1">
						<input type="hidden" name="currency_code" value="<?=$currency;?>">
						<input type="hidden" name="bn" value="FC-BuyNow">
						<input type="hidden" name="return" value="<?=base_url()?>tinvoices/view/<?=$invoice->id;?>">
						<input type="hidden" name="cancel_return" value="<?=base_url()?>tinvoices/view/<?=$invoice->id;?>">
	...
	<div class="btn-group dropup">
      <?php if (isset($isSignedUp->AccountNumber) && $isSignedUp->AccountNumber > 0 && isset($isSignedUp->ProfileId) && $isSignedUp->ProfileId != null && $isSignedUp->ProfileId > 0) : ?>
      ...
      <?php if($core_settings->bank_transfer == "1" && $sum != "0.00" && $invoice->status != "Paid" ){ ?>
	    <li><a id="pay_bank_transfer" data-toggle="mainmodal" href="<?=base_url()?>tinvoices/banktransfer/<?=$invoice->id;?>/<?=$sumRest;?>"><?=$this->lang->line('application_bank_transfer');?></a></li>
	  <?php } ?>
	  ...
	  <?php } ?>
      -->
      <li><a id="pay_credit_card" data-toggle="mainmodal" href="<?=base_url()?>tinvoices/propay/<?=$invoice->id;?>/<?=$sumRest;?>?paymentType=card"><?=$this->lang->line('application_credit_card');?></a></li>
      <li><a id="pay_check" data-toggle="mainmodal" href="<?=base_url()?>tinvoices/propay/<?=$invoice->id;?>/<?=$sumRest;?>?paymentType=ach"><?=$this->lang->line('application_check');?></a></li>
      <!--
	  <?php if($core_settings->stripe == "1" && $core_settings->twocheckout == "0" && $core_settings->authorize_net == "0" && $sum != "0.00" && $invoice->status != "Paid" ){ ?>  
	    ...
	    <li><a id="pay_credit_card" data-toggle="mainmodal" href="<?=base_url()?>tinvoices/stripepay/<?=$invoice->id;?>/<?=$sumRest;?>"><?=$this->lang->line('application_credit_card');?></a></li>
	    	<?php if($core_settings->stripe_ideal == "1") { ?>
	    		<li><a id="pay_ideal" data-toggle="mainmodal" href="<?=base_url()?>tinvoices/stripepay/<?=$invoice->id;?>/<?=$sumRest;?>/ideal"><?=$this->lang->line('application_iDEAL');?></a></li>
	    	<?php } ?>
	  <?php } ?>
	  ...
	  <?php if($core_settings->twocheckout == "1" && $core_settings->stripe == "0" && $core_settings->authorize_net == "0" && $sum != "0.00" && $invoice->status != "Paid" ){ ?>  
	    ...
	    <li><a id="pay_credit_card" data-toggle="mainmodal" href="<?=base_url()?>tinvoices/twocheckout/<?=$invoice->id;?>/<?=$sumRest;?>"><?=$this->lang->line('application_credit_card');?></a></li>
	  <?php } ?>

	  <?php if($core_settings->stripe == "0" && $core_settings->twocheckout == "0" && $core_settings->authorize_net == "1" && $sum != "0.00" && $invoice->status != "Paid" ){ ?>  
	    <li><a id="pay_credit_card" data-toggle="mainmodal" href="<?=base_url()?>tinvoices/authorizenet/<?=$invoice->id;?>/<?=$sumRest;?>"><?=$this->lang->line('application_credit_card');?></a></li>
	  <?php } ?>-->
	  </ul>
      <?php endif;?>
	</div>
