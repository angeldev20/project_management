...
<?=form_open($form_action, $attributes)?>
        <div class="logo"><img style="max-height: 50px; max-width: 200px; width: auto;" src="https://spera-<?=ENVIRONMENT ?>.s3-us-west-2.amazonaws.com/<?=$_SESSION['accountUrlPrefix']?>/<?php if($core_settings->login_logo == ""){ echo $core_settings->invoice_logo;} else{ echo $core_settings->login_logo; }?>" alt="<?=$core_settings->company;?>"></div>
        <?php if($error != 'false') { ?>
...
<div class="header"><?=$this->lang->line('application_enter_your_details_to_create_an_account');?><hr></div>
  ...
  <div class="col-md-6"> 
    ...
    <div class="form-group">
            ...
            <input autocomplete="off" id="password" type="password" name="password" class="form-control" value="" required />
    </div>
    <div class="form-group">
            <label for="password"><?=$this->lang->line('application_confirm_password');?> *</label>
            <input autocomplete="off" id="confirm_password" type="password" class="form-control" data-match="#password" required />
    </div>
