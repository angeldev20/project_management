	...
	<div class="grid__col-sm-12 grid__col-md-3 grid__col-lg-3">
		...
		<div class="subcont">
			...
			<?php $lable = FALSE; if($ticket->status == "new"){ $lable = "label-important"; }elseif($ticket->status == "open"){$lable = "label-warning";}elseif($ticket->status == "closed" || $ticket->status == "inprogress"){$lable = "label-success";}elseif($ticket->status == "reopened"){$lable = "label-warning";} ?>
				...
				<li><span><?=$this->lang->line('application_type');?></span> <?php if(is_object($ticket->type)){ ?><?=$ticket->type->name;?> <?php } ?></li>
				<li><span><?=$this->lang->line('application_from');?></span> 
					<?php if(is_object($ticket->client)){ echo '<a class="tt" title="'.$ticket->client->email.'">'.$ticket->client->firstname.' '.$ticket->client->lastname.'</a>';
				...
				<li>
					...
					<?php if(is_object($ticket->queue)){ ?><?=$ticket->queue->name;?> <?php } ?>
				</li>
				<li>
					<span>
				...
				<li>
					<span><?=$this->lang->line('application_owner');?></span> <?php if(is_object($ticket->user)){ ?><?=$ticket->user->firstname;?> <?=$ticket->user->lastname;?> <?php } else{ echo "-";} ?>
				</li>
			</ul>
	 	</div>
	 	<br>
		...
		<div class="subcont">
			<ul class="details">
				<?php if(is_object($ticket->client)){ ?><li><span><?=$this->lang->line('application_name');?>:</span>  <?php echo $ticket->client->firstname.' '.$ticket->client->lastname; ?> </li><?php } ?>
				<li><span><?=$this->lang->line('application_company');?>:</span> <?php if(!is_object($ticket->client)){ ?> <a href="#" class="label label-info"><?php echo $this->lang->line('application_no_client_assigned'); }else{ ?><a class="label label-info" href="<?=base_url()?>clients/view/<?=$ticket->client->company->id;?>"><?php echo $ticket->client->company->name;} ?></a></li>
				<?php if(is_object($ticket->client) && !empty($ticket->client->email)){ ?> <li><span><?=$this->lang->line('application_email');?>:</span> <?=$ticket->client->email;?></li><?php } ?>
				<?php if(is_object($ticket->client) && !empty($ticket->client->phone)){ ?> <li><span><?=$this->lang->line('application_phone');?>:</span> <?php echo $ticket->client->phone; ?></li><?php } ?>
				<?php if(is_object($ticket->client) && !empty($ticket->client->mobile)){ ?> <li><span><?=$this->lang->line('application_mobile');?>:</span> <?php echo $ticket->client->mobile; ?></li><?php } ?>
			</ul>
		</div>
	</div>
	 <div class="grid__col-sm-12 grid__col-md-9 grid__col-lg-9">
		...
		<?php
			...
			<?php }
		?>	
			<div class="article-content" id="article-<?=$value->id?>">
				<div class="article-header">
					<div class="article-title">
						<?=$value->subject;?> <?php if($value->internal == "0"){ ?><i class="icon dripicons-preview tt pull-right" title="<?=$this->lang->line('application_task_public');?>"> </i> <?php } ?>
					</div>
...
<script>
    jQuery(function($) {
        ...
        if(article_id != '-1') {
            $("html, body").animate({
                scrollTop: $("#article-" + article_id).offset().top
            });
        }
    });
</script>