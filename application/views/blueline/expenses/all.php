    ...
    foreach ($expenses as $value):
      ...
      <td><?php if($value->attachment != ""){echo '<a href="'.base_url().'expenses/attachment/'.$value->id.'" style="color: #505458;"><i class="icon dripicons-paperclip tt" title="'.$value->attachment_description.'"></i></a>'; }?></td>
   <!--   <td class="hidden-xs"><?php if(!empty($value->status)){echo $this->lang->line('application_'.$value->status); } ?> </td> -->
      ...
      <td class="hidden-xs">
        ...
        <span class="<?php if(is_object($value->invoice)){echo 'plus tt" title="'.$this->lang->line("application_rebilled_on_invoice").' #'.$value->invoice->reference; }?>"><?=display_money(sprintf("%01.2f", round($value->value, 2)));?></span>
      </td>
      <td class="hidden-xs minus ">
        <span><?=display_money(sprintf("%01.2f", round($value->value-(($value->value/(100+$value->vat))*100), 2)));?></span>
      </td>
