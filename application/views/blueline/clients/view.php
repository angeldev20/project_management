 <div class="row">
	...
	<div class="col-md-12">
		...
		<div class="row">
			<div class="col-md-3 marginbottom20">
				<div class="table-head">
					<?=$this->lang->line('application_company_details');?> <span class="pull-right"><a href="<?=base_url()?>clients/company/update/<?=$company->id;?>/view" class="btn btn-primary" data-toggle="mainmodal"><i class="icon-edit"></i> <?=$this->lang->line('application_edit');?></a>
				</div>
			<div class="subcont">
			<ul class="details col-md-12">
				...
				<li><span><?=$this->lang->line('application_primary_contact');?>:</span> <?php if(is_object($company->client)){ echo $company->client->firstname.' '.$company->client->lastname;}else{echo "-";} ?></li>
				<li><span><?=$this->lang->line('application_email');?>:</span> <?php if(is_object($company->client) && $company->client->email != ""){ echo $company->client->email; }else{ echo "-"; } ?></li>
			...
			<ul class="details col-md-12">
				...
				<li>
					<span><?=$this->lang->line('application_social_media');?>:</span>
	 	...
	 	<div class="col-md-9">
	 		<?php if(!array_key_exists(0, $company->clients)){ ?><div class="alert alert-warning"><?=$this->lang->line('application_client_has_no_contacts');?> <a href="<?=base_url()?>clients/create/<?=$company->id;?>" data-toggle="mainmodal"><?=$this->lang->line('application_add_new_contact');?></a></div>
	 		<?php } ?>
		...
		<thead>
			...
			<th class="hidden-xs"><?=$this->lang->line('application_social_media');?></th>
		...
		<tr id="<?=$value->id;?>" >
			...
			<td class="option" style="text-align:left; text-wrap:nowrap " width="9%">
						<a  href="<?=base_url()?>clients/credentials/<?=$value->id;?>" class="btn-option tt" title="<?=$this->lang->line('application_email_login_details');?>" data-toggle="mainmodal"><i class="icon dripicons-mail"></i></a>
				        ...
				        <a href="<?=base_url()?>clients/update/<?=$value->id;?>" title="<?=$this->lang->line('application_edit');?>" class="btn-option" data-toggle="mainmodal"><i class="icon dripicons-gear"></i></a>
			</td>
		</tr>
		...
		<?php endforeach;?>
	 	...
	 	<div class="col-md-9"> 
	 		<?php if(!array_key_exists(0, $company->users)){ ?><div class="alert alert-warning"><?=$this->lang->line('application_client_has_no_admins');?> <?php if($this->user->admin == 1){ ?><a href="<?=base_url()?>clients/assign/<?=$company->id;?>" data-toggle="mainmodal"><?=$this->lang->line('application_assign_admin');?></a><?php } ?></div>
	 		<?php } ?>
	 	<div class="data-table-marginbottom">

		<div class="table-head"><?=$this->lang->line('application_client_admins');?> <?php if($this->user->admin == 1){ ?><span class="pull-right"><a href="<?=base_url()?>clients/assign/<?=$company->id;?>" class="btn btn-primary" data-toggle="mainmodal"><?=$this->lang->line('application_assign_admin');?></a></span><?php } ?></div>
		<div class="table-div">
		...
		<thead>
			...
			<th class="hidden-xs"><?=$this->lang->line('application_email');?></th>
			<th class="hidden-xs"><?=$this->lang->line('application_last_login');?></th>
			<?php if($this->user->admin == 1){ ?><th><?=$this->lang->line('application_action');?></th><?php } ?>
		</thead>
...
<?php } ?>
        ...
        <div class="no-files">  
            <i class="icon dripicons-lightbulb"></i><br>
