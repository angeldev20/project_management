<div class="col-sm-12  col-md-12 main">  
	<div id="options" class="row">
		...
		<tr id="<?=$value->id;?>" >
			...
			<td class="hidden-xs"><span class="label <?php if(is_object($value->user)){echo 'label-info">'.$value->user->firstname.' '.$value->user->lastname;}else{echo '">'.$this->lang->line('application_not_assigned');}?> </span></td>
		...
		<tr id="<?=$value->id;?>" >
			<td class="hidden-xs"><?=$core_settings->quotation_prefix;?><?=$value->id;?></td>
			<td><span class="label <?php if($value->status == "New"){echo "label-important";}elseif($value->status == "Accepted"){ echo "label-success";}elseif($value->status == "Reviewed"){ echo "label-warning";} ?>"><?=$this->lang->line('application_'.$value->status);?></span></td>
			<td class="hidden-xs"><?php  if(is_object($value->customquote)){echo $value->customquote->name;}else{echo "-";}?></td>
			<td><?php $unix = human_to_unix($value->date); echo '<span class="hidden">'.$unix.'</span> '; echo date($core_settings->date_format.' '.$core_settings->date_time_format, $unix); ?></td>
			<td  class="hidden-xs"><span class="label <?php if(is_object($value->user)){echo 'label-info">'.$value->user->firstname; echo " ".$value->user->lastname;}else{echo '">'.$this->lang->line('application_not_assigned');}?></span></td>
