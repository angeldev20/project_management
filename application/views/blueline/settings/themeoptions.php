<div id="row" class="grid">
	...
	<div class="grid__col-sm-12 grid__col-md-9 grid__col-lg-9">
		<div class="table-head"><?=$this->lang->line('application_theme_options');?></div>
		<div class="table-div settings">
		<?php   
		$attributes = array('class' => '', 'id' => 'theme_options');
		echo form_open_multipart($form_action, $attributes); 
		?>

	<div class="form-header"><?=$this->lang->line('application_theme_colors');?></div>

	<div class="row">
		<div class="col-md-12">
			<div class="form-group">
				<label><?=$this->lang->line('application_activate_custom_colors');?></label>
	            <input name="custom_colors" type="checkbox" class="checkbox" style="width:100%;" data-labelauty="<?=$this->lang->line('application_activate_custom_colors');?>" value="1" <?php if($settings->custom_colors == "1"){ ?> checked="checked" <?php } ?>>
 			</div>
		</div>
		<div class="col-md-4">		
			<div class="form-group">
					<label><?=$this->lang->line('application_top_bar_background');?></label>
		...
		<div class="col-md-4">
			<div class="form-group">
					<label><?=$this->lang->line('application_top_bar_color');?></label>
		...
		<div class="col-md-4">
			<div class="form-group">
					<label><?=$this->lang->line('application_body_background');?></label>
		...
		<div class="col-md-4">
			<div class="form-group">
					<label><?=$this->lang->line('application_menu_background');?></label>
		...
		<div class="col-md-4">
			<div class="form-group">
					<label><?=$this->lang->line('application_menu_color');?></label>
		...
		<div class="col-md-4">
			<div class="form-group">
					<label><?=$this->lang->line('application_primary_color');?></label>
	...
	<div class="form-header"><?=$this->lang->line('application_login_page');?></div>
        ...
        <div class="form-group">
			<label><?=$this->lang->line('application_login');?> <?=$this->lang->line('application_logo');?>
				...
				<?php if($core_settings->login_logo != ""){ ?>
						<button type="button" class="btn-option po " data-toggle="popover" data-placement="right"  data-content="<div style='padding:10px'><img src='https://spera-<?=ENVIRONMENT ?>.s3-us-west-2.amazonaws.com/<?=$_SESSION['accountUrlPrefix']?>/<?php if($core_settings->login_logo == ""){ echo $core_settings->invoice_logo;} else{ echo $core_settings->login_logo; }?>'></div>" data-original-title="<?=$this->lang->line('application_login');?> <?=$this->lang->line('application_logo');?>"> <i class="ion-eye"></i></button>
						<?php } ?>
			</label>
	                <div>
		...
		<div class="form-group">
			 	<label><?=$this->lang->line('application_login_background');?> </label>
			 	<select name="login_background" id="login_background" class="formcontrol chosen-select ">
						<?php foreach ($backgrounds as $value) { ?>
							<option value="<?=$value;?>"  <?php if($settings->login_background == $value){ ?>selected=""<?php } ?> ><?=$value;?></option>
						<?php } ?>
				</select>
        </div>
        <div class="form-group">
			<label><?=$this->lang->line('application_upload_background');?></label>
	                <div>
		...
		<div class="form-group">
			 	<label><?=$this->lang->line('application_login_style');?> </label>
			 	<select name="login_style" id="login_style" class="formcontrol chosen-select ">
							<option value="left"  <?php if($settings->login_style == "left"){ ?>selected=""<?php } ?> >
								<?=$this->lang->line('application_left');?>
							</option>
							<option value="center"  <?php if($settings->login_style == "center"){ ?>selected=""<?php } ?> >
		...
		<div class="image-page-frame">
			<img class="hidden-xs <?php if($settings->login_style != "left"){ echo "hidden"; }?>" id="login_style_left" style="max-height:200px" src="<?=base_url()?>/assets/blueline/images/login_side.jpg">
			<img class="hidden-xs <?php if($settings->login_style != "center"){ echo "hidden"; }?>" id="login_style_center" style="max-height:130px; margin-top:25px; margin-left: 110px; position:absolute;" src="<?=base_url()?>/assets/blueline/images/login_side.jpg">
			<img class="hidden-xs" id="background_preview" style="max-height:200px" src="https://spera-<?=ENVIRONMENT ?>.s3-us-west-2.amazonaws.com/<?=$_SESSION['accountUrlPrefix']?>/assets/blueline/images/backgrounds/<?=$settings->login_background;?>">
            <img class="visible-xs img-responsive" src="https://spera-<?=ENVIRONMENT ?>.s3-us-west-2.amazonaws.com/<?=$_SESSION['accountUrlPrefix']?>/assets/blueline/images/backgrounds/<?=$settings->login_background;?>">
		</div>

<script>
    $(function() {
    	var colors = {'#161b1f': '#161b1f',
                '#d8dce3': '#d8dce3',
                '#11a7db': '#11a7db',
                '#2aa96b': '#2aa96b',
                '#5bc0de': '#5bc0de',
...
<script>
	$('#login_background').on('change', function(evt, params) {
			$("#background_preview").attr("src","https://spera-<?=ENVIRONMENT ?>.s3-us-west-2.amazonaws.com/<?=$_SESSION['accountUrlPrefix']?>/assets/blueline/images/backgrounds/"+params.selected);
	 });
