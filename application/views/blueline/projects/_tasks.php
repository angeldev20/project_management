<?php   
$attributes = array('class' => 'dynamic-form1', 'data-reload' => 'task-list', 'data-reload2' => 'milestones-list', 'data-reload3' => 'taskviewer-content', 'data-baseurl' => base_url(), 'id' => '_task');
...
<div class="row">
 ...
 <div class="form-group">
        ...
        <?php $users = array();
                //TODO:  this is a hack for now, we need a better way to grab the first administrative user
                //and push it through to the front end
                $users['9'] = '-';
        ...
        if(isset($task)){
            //$user = $task->user_id;
            $user = [];
            foreach($task->task_has_workers as $worker) {
                if(isset($users[$worker->user->id]))
                    $user[] = $worker->user->id;
            }
...
<ul class="accesslist">
        ...
        <div class="modal-footer">
          <?php if(isset($task)){ ?>
            <a href="<?=base_url()?>projects/task/<?=$task->project_id;?>/delete/<?=$task->id;?>" class="btn btn-danger pull-left button-loader" ><?=$this->lang->line('application_delete');?></a>
          <?php }else{  ?>
        ...
        <?php } ?>
        <button name="send" class="btn btn-success"><?=$this->lang->line('application_save');?></button>
        </div>
<?php echo form_close(); ?>