    ...
    <div class="row tile-row">
     ...
     <div class="row">
      ...
      <div class="btn-group pull-right margin-right-3">
          <?php if (isset($isAdmin)  && $isAdmin != 0) : ?>
          <a href="<?=base_url()?>projects/export" class="btn btn-primary" data-toggle="mainmodal"><?=$this->lang->line('application_export');?></a>
          <?php endif; ?>
      </div>
    </div>
      <div class="row">
         ...
         <table class="data table" id="projects" rel="<?=base_url()?>" cellspacing="0" cellpadding="0">
                ...
                <tr id="<?=$value->id;?>">
                  ...
                  <td class="hidden-xs"><a class="label label-info"><?php if(!is_object($value->company)){echo $this->lang->line('application_no_client_assigned'); }else{ echo $value->company->name; }?></a></td>
                  ...
                  <td class="hidden-xs">
                    <?php 
                      ...
                      ?>
                      <?php if($image) { ?>
                      <img class="img-circle tt" src="<?=$image;?>" title="<?php echo $workers->user->firstname.' '.$workers->user->lastname;?>" height="19"><span class="hidden"><?php echo $workers->user->firstname.' '.$workers->user->lastname;?></span>
                      <?php } else { ?>
                      <a class="initials-avatar-circle rainbow tt" title="<?php echo $workers->user->firstname.' '.$workers->user->lastname;?>"><?php echo strtoupper(substr($workers->user->firstname, 0, 1).substr($workers->user->lastname, 0, 1)); ?></a>&nbsp;
                      <?php } ?>
                    <?php endforeach;?>
                  </td>
