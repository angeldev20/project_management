                    ...
                    <tr id="<?= $value->id; ?>">
                        ...
                        <td class="hidden-xs">
                            ...
                            <div class="circular-bar tt" title="<?= $value->progress; ?>%">
                                <input type="hidden" class="dial"
                                       data-fgColor="<?php if ( $value->progress == "100" ) { ?>#43AC6E<?php } else { ?>#11A7DB<?php } ?>"
                                       data-width="19" data-height="19" data-bgColor="#e6eaed"
                        ...
                        <td class="hidden-xs">
							<?php foreach ( $value->project_has_workers as $workers ): ?>
								...
								if ( $image ) { ?>
                                    <img class="img-circle tt" src="<?= $image; ?>"
								...
								<?php } else { ?>
                                    <a class="initials-avatar-circle rainbow tt"
                                       title="<?php echo $workers->user->firstname . ' ' . $workers->user->lastname; ?>"><?php echo strtoupper( substr( $workers->user->firstname, 0, 1 ) . substr( $workers->user->lastname, 0, 1 ) ); ?></a>&nbsp;
								<?php } ?>
							<?php endforeach; ?>
                        </td>
