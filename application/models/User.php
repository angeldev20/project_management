...
class User extends ActiveRecord\Model
{
	static $has_many = array(
	     ...
	     array('quotes'),
	     array('quoterequests'),
         array('slack_links', 'readonly' => true),
    ...
    /**
     * This is an api function to validate the api token
     * @param string $hashed_password
     * @param string $username
     * @param int $expires - unix timestamp int
     * @return bool
     */
	public static function validate_token($hashed_password, $username, $expires) {
        if ($expires < time()) FALSE;
	    $user = User::find_by_username_and_status($username, "active");
	    if (!$user) $user = User::find_by_email_and_status($username, "active");
	...
	public static function validate_gmail($gmail)
	{
		$user = User::find_by_email_and_status($gmail, "active");
		if($user)
		{
			User::login($user->id, 'user_id');
			$update = User::find($user->id);
