			...
			$item = Quote::create($_POST);
       		if(!$item){$this->session->set_flashdata('message', 'error:'.$this->lang->line('quotation_create_error'));}
					...
					send_user_notification(
					    $this->user,
					    $value->email,
                        $this->lang->line('application_notification_quotation_subject'),
                        $this->lang->line('application_notification_quotation'),
                        false,
                        'estimates/view/' . $item->id
                    );
					endforeach;
       		}
			redirect('quotation');
			    ...
			    $required = '';
			      ...
			      case 'section_break':
			        ...
			        $html_fields .= '<label class="control-label">'.$field->label.'</label>';
			          ...
			          $html_fields .= '<div><label class="margin-bottom-10"><input type="radio" data-labelauty="'.$value->label.'" name="'.$field->cid.'" value="'.$value->label.'" '.$required.'/>&nbsp;'.$value->label.'</label></div>';
			  			}
			      ...
			      case 'checkboxes':
			        ...
			        foreach ($field->field_options->options as $value) {
			          ...
			          $html_fields .= '<div><label class="margin-bottom-10"><input type="checkbox" data-labelauty="'.$value->label.'" name="'.$field->cid.'_'.$i.'" value="'.$value->label.'" '.$checked.' />&nbsp;'.$value->label.'</label></div>';
