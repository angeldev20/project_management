	...
	{	
		...
		$data["core_settings"] = Setting::first();
			...
			send_user_notification(
			    $this->client,
			    $data["core_settings"]->email,
                $data["core_settings"]->estimate_prefix.$this->view_data['estimate']->estimate_reference.' - '.$this->lang->line('application_Accepted'),
                $this->lang->line('messages_estimate_accepted'),
                false,
                base_url() . 'cestimates/view/'.$id
            );
	...
	{	
		...
		if($_POST){
			...
			send_user_notification(
			    $this->client,
			    $data["core_settings"]->email,
                $data["core_settings"]->estimate_prefix.$this->view_data['estimate']->estimate_reference.' - '.$this->lang->line('application_Declined'), $_POST['reason'],
                false,
                base_url() . 'cestimates/view/'.$_POST['invoice_id']
            );
