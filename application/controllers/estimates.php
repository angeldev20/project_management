		...
		if ( $this->user->admin == 0 ) {
			...
			if ( ! empty( $thisUserHasNoCompanies ) ) {
				...
				if ( $status && $status != 'All' ) {
					$options = array(
						'conditions' => array(
							'status = ? AND estimate != ? AND company_id in (?) AND id NOT IN (SELECT invoice_id FROM invoice_has_users)',
							$status,
							0,
							$comp_array
						)
...
GROUP BY paid_date" );
	...
	function store() {
		if ( $_POST ) {
			$allowed = ['estimate_status', 'currency'];
			...
			redirect( 'estimates/edit/'.$invoice->id );	
		}
		die();
	}
		...
		if ( $_POST ) {
			...
			if ( $this->user->admin != 1 ) {
				...
				if ( ! empty( $thisUserHasNoCompanies ) ) {
					...
					$this->view_data['estimates'] = Invoice::find( 'all', array(
						'conditions' => array(
							'company_id in (?)',
							$comp_array
						)
	...
	function update( $id = false )
	{
		...
		if ( $_POST ) {
			...
			if(isset($data['estimate_status']) && $data['estimate_status'] == 'Paid') {
				$data['paid_date'] = date("Y-m-d");
			}

			if(isset($data['estimate_status']) && $data['estimate_status'] == 'Sent') {
	...
	function update1( $id = false, $getview = false )
	{
		if ( $_POST ) {
			...
			if ( $_POST['company_id'] == '0' ) {
				$this->session->set_flashdata( 'message', 'error:Please select client' );
				$error = true;
			} else if ( $_POST['project_id'] == '0' ) {
				$this->session->set_flashdata( 'message', 'error:Please select project' );
				$error = true;
			}
	...
	function view( $id = false )
	{
		...
		$this->view_data['items'] = InvoiceHasItem::find( 'all', array(
			'conditions' => array(
				'invoice_id=?',
				$id
			)
