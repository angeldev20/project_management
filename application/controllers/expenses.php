...
class Expenses extends MY_Controller {
	...
	function data()
	{
		...
		$data = array_map(function($expense) {
			...
			if(!empty($expense['project_id'])) {
				$expense['project'] = Project::find($expense['project_id'])->attributes();
			}
			if(!empty($expense['invoice_id'])) {
				$expense['invoice'] = Invoice::find($expense['invoice_id'])->attributes();
			}
			if(!empty($expense['user_id'])) {
				$expense['user'] = User::find($expense['user_id'])->attributes();
				$expense['user']['userpic'] = get_user_pic($expense['user']['userpic']);
			}
			return $expense;
		}, $data);
	...
	function edit($id = false) {
		if ( $_POST ) {
			...
			$expense = $expense->update_attributes($_POST);
