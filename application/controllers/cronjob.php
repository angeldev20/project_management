...
class Cronjob extends MY_Controller
{
	function index()
	{
		...
		if($core_settings->autobackup == "1" && time() > ($core_settings->last_autobackup + 7 * 24 * 60 * 60)){
			...
			$prefs = array('format' => 'zip', 'filename' => 'Database-auto-full-backup_'.$version.'_'.date('Y-m-d_H-i'));
		...
		$expenses = Expense::find_by_sql('SELECT * FROM expenses WHERE type = "recurring_payment" AND (recurring_until > "'.$date.'" OR recurring_until = "") AND "'.$date.'" >= next_payment ORDER BY next_payment');
		...
		if(!$invoice){
			...
			$core_settings->update_attributes(array('invoice_reference' => $invoice_reference+1));
		...
		if(is_object($invoice->company) && is_object($invoice->company->client) && $invoice->company->client->email != ""){
			...
			$parse_data = array(
	        					...
	        					'logo' => '<img src="https://spera-' . ENVIRONMENT . '.s3-us-west-2.amazonaws.com/' . $_SESSION["accountUrlPrefix"] . '/' . $core_settings->logo.'" alt="'.$core_settings->company.'"/>',
	        					'invoice_logo' => '<img src="https://spera-' . ENVIRONMENT . '.s3-us-west-2.amazonaws.com/' . $_SESSION["accountUrlPrefix"] . '/' . $core_settings->invoice_logo.'" alt="'.$core_settings->company.'"/>'
			);
