					...
					try {
					    ...
					    if ($charge['response']['responseCode'] == 'APPROVED') {
							...
							send_user_notification(
                                ...
                                $this->lang->line('application_notification_payment_processed_subject'),
                                $this->lang->line('application_notification_payment_processed').' #'.$data["core_settings"]->invoice_prefix.$invoiceHasPayment->invoice->reference,
                                false,
                                base_url . 'cinvoices/view/'.$_POST['id']
                            );
		...
		try {
			...
			if ($charge->paid == true) {
				...
				send_user_notification(
                    ...
                    $this->lang->line('application_notification_payment_processed').' #'.$data["core_settings"]->invoice_prefix.$invoiceHasPayment->invoice->reference,
                    false,
                    base_url() . 'invoices/view/'. $invoice->id
                );
		...
		if($_POST){
				...
				if( $this->authorize_net->authorizeAndCapture() )
				{
					...
					send_user_notification(
                        ...
                        $this->lang->line('application_notification_payment_processed').' #'.$data["core_settings"]->invoice_prefix.$invoiceHasPayment->invoice->reference,
                        false,
                        base_url() . 'cinvoices/view/'.$invoice->id
                    );
