...
class Register extends MY_Controller
{
	...
	{	
		...
		if($_POST)
		{
			...
			if(!isset($client) || $client->inactive == 1){$client = FALSE;}
			...
			if(!$client && !$check_company && trim(htmlspecialchars($_POST['name'])) != "" && trim(htmlspecialchars($_POST['email'])) != "" && $_POST['password'] != "" && $_POST['firstname'] != "" && $_POST['lastname'] != "" && $_POST['confirmcaptcha'] != ""){ 
				...
				if($client){
            		...
            		$tickets = Ticket::find("all", array('conditions' => array( '`from` LIKE CONCAT("%", ? ,"%")', $client_attr['email'] ))); 
            		if($tickets){
				...
				if($client){$this->view_data['error'] = $this->lang->line('messages_email_already_taken');}
				if($check_company){$this->view_data['error'] = "Company name is already taken!";}
