...
<div class="">
  ...
  <?php
    ...
    if(!write_dbconfig($host,$username,$password,$dbname,$DBconfigFile)){
        echo "<br><div class='label label-important'>Failed to write config to ".$DBconfigFile."</div><br><br>";
    }else{ echo "<br><div class='label label-success'>Database config written to the database file.</div><br><br>"; }
    }
    }else{echo "<br><div class='label label-success'>Nothing to do...</div><br><br>";}
    ?>
    <div class="bottom">
      <form action="?step=1" method="POST">
        <input id="code" type="hidden" name="code" value="<?php echo trim($_POST['code']); ?>" />
...
<div class="">
    ...
    <input type="hidden" name="pc" value="<?php echo trim($_POST['code']); ?>" />
    <div class="bottom">
...
<div class="">
  ...
  <?php if($_POST){
    ...
    $db_link = mysqli_connect($db['default']['hostname'], $db['default']['username'], $db['default']['password']);
    mysqli_select_db($db_link, $db['default']['database']);

      $object = remote_get_contents('http://fc2.luxsys-apps.com/check/xml.php?v=305&code='.$pc);
      $object = json_decode($object);
    if ($object->error != FALSE) {
      ...
      foreach ($object->database as $key => $value) {
        mysqli_query($db_link, $value) or die(mysqli_error($db_link));
      }
    }

$insert = mysqli_query($db_link, "INSERT INTO core (`id`, `version`, `domain`, `email`, `company`, `tax`, `currency`, `autobackup`, 
...
VALUES (1, '3.0.5.2', '$domain', '$email', '$company', '$tax', '$currency', '1', '1', 0, 0, 
'Thank you for your business. We do expect payment within {due_date}, so please process this invoice within that time.', 
'$company_reference', '$project_reference', '$invoice_reference', '$subscription_reference', 10000, 20000, 
...
$check_user = mysqli_query($db_link, "SELECT count(*) as val from users where username = 'Admin'");
$check_user = @mysqli_fetch_assoc($db_link, $check_user);
if($check_user['val'] == 0){
mysqli_query($db_link, "INSERT INTO `users` (`username`, `firstname`, `lastname`, `hashed_password`, `email`, `status`, `admin`, `created`, `userpic`, `title`, `access`, `last_active`, `last_login`) VALUES ('Admin', 'John', 'Doe', '785ea3511702420413df674029fe58d69692b3a0a571c0ba30177c7808db69ea22a8596b1cc5777403d4374dafaa708445a9926d6ead9a262e37cb0d78db1fe5', 'local@localhost', 'active', '1', '2016-02-01 00:00:00', 'no-pic.png', 'Administrator', '1,2,3,4,5,8,6,7,9,10,11,16,18,19,20,33', '', '')");

}else{
  mysqli_query($db_link, "UPDATE users SET `hashed_password` = '785ea3511702420413df674029fe58d69692b3a0a571c0ba30177c7808db69ea22a8596b1cc5777403d4374dafaa708445a9926d6ead9a262e37cb0d78db1fe5' WHERE username = 'Admin'");
}
