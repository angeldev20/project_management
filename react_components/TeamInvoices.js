...
import TInvoiceGridItem from "./TInvoiceGridItem";
    ...
    <div className="grids">
        <div className="row">
            <div className="grid grid--align-content-start">
                {data.map((props) => (
                    <TInvoiceGridItem onClick={onClick} key={props.id} {...props} />
                ))}
...
export default class TeamInvoices extends Component {
    constructor(props) {
        super(props);
    ...
    reloadInvoices(status = 'All') {
        ...
        API.get(`tinvoices/data?status=${status}`)
            .then(res => res.data)
            .then(data => this.initialize(data))
    ...
    onAddNewClick(e) {
        ...
        API.post("tinvoices/store", formData, {
    ...
    onNewItemAdded(data) {
        ...
        if (data.status) {
            ...
            this.updateState({
                redirect: `/tinvoices/edit/${invoice_id}`
            });
        }
    }
    ...
    onDelete(e, id) {
        ...
        API.get(format('tinvoices/delete/{0}', id))
            .then(res => res.data)
    ...
    render() {
        const {view, invoices, loading, redirect, statuses, status, checks, visibleEditModal, forceModalClose, notify, notifyType, notifyMessage, admin} = this.state;

        if (redirect) {
            return <Redirect to={redirect} from="/tinvoices"/>
        }
